...
description: "未上市公司研究：多Agent并行深度研究框架。专为蚂蚁集团、小红书、SpaceX、Stripe等未上市公司设计，通过6个并行Agent从商业模式、财务、竞争、风险、技术、替代数据多维度拼凑信息，还原真实价值。"
...
4. **精确算术工具**：市值计算、估值、跨源校验使用精确工具 `tools/common/financial_rigor.py`，**禁止 LLM 心算**决策敏感数据。
...
> 跨币种估值对比时，先用 `python tools/common/fx_rate.py --code USDCNY`（或 `HKDCNY`）获取实时汇率，勿用固定汇率。估值计算须使用 `tools/common/financial_rigor.py`，禁止 LLM 心算。
...
将完整最终报告写入 `reports/{公司名}/{公司名}-private-{YYYYMMDD}.md`。如果 `reports/{公司名}/` 目录不存在则创建。
...
python tools/common/report_audit.py extract \
  --report reports/{公司名}/{公司名}-private-{YYYYMMDD}.md
...
1. **6个Agent必须并行启动**——在同一条消息中调用6次 Task 工具
