...
**重要流程**：对于A股公司，必须首先使用 `report_hub.py ensure` 工具获取原始财报PDF，**确认返回 success=true 后方可进行下一步的阅读分析工作**。
...
获取的PDF文件统一保存到 `cninfo_reports/` 目录，文件命名格式：
...
**注意**：网络搜索须使用本地五工具组合（详见 [web-search-tools](../tools-scripts/web-search-tools.md)），美股深度数据须 exa + doubao 双源验证。
...
财报精读的核心数据来源是一手财报PDF。报告获取与提取统一使用 report_hub.py（带下载与提取缓存），详见 [报告下载与提取统一入口](../tools-scripts/report-hub.md)。A股使用 `tools/common/report_hub.py`：
...
### PDF文档阅读工具（经 report_hub extract 统一调用）

A股财报提取文字与表格统一使用 `tools/common/report_hub.py extract`（内置类型检测、自动乱码检测 + OCR 回退与提取缓存），返回失败时才按 report-hub.md 回退 Poppler 工具集：

| 工具 | 功能 | 命令示例 |
|------|------|---------|
| `report_hub.py extract` | PDF文字与表格提取（统一入口，带缓存） | `python tools/common/report_hub.py extract --code 002465 --report-type annual` |

**注意**：首选 `report_hub.py extract`，返回失败时才回退 Poppler（命令详见 [报告下载与提取统一入口](../tools-scripts/report-hub.md)）；如果PDF无法提取内容，应在报告中标注"资料评级：B级"，说明扫描版PDF限制。
