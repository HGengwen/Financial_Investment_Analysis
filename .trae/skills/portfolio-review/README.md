...
长期组合审视，聚焦集中度、相关性、机会成本与压力测试四维度。`--horizon long` 为默认值，**可省略**；不传参数时行为与显式传 `--horizon long` 完全一致。
...
中期组合审视（底仓/机动仓分离 + "3+2"结构）：

- 中期审视：`/portfolio-review {持仓清单} --horizon mid`
- 显式标注底仓/机动仓：`/portfolio-review {持仓清单} --horizon mid 底仓标注：腾讯=底仓, 英伟达=机动仓`
...
长期组合审视是默认行为，聚焦集中度、相关性、机会成本与压力测试四维度，组合文件输出至 `reports/portfolio-{YYYYMMDD}.md`。
...
| --- | --- | --- |
| 仓位结构 | 无底仓/机动仓之分 | 底仓 60%~80% + 机动仓 20%~40% 分离校验 |
| 单只上限 | 第一大 <40% | 初始 ≤10%、验证后 ≤15%、绝对上限 ≤20% |
| 行业上限 | — | 单行业 ≤40% |
| 集中度建议 | 前三大 50-80%、5-15 只 | "3+2"结构（3 核心 12%~15% + 2~3 卫星 5%~8%） |
| 机会成本 | 预期回报×确定性 | 加入 PEG 水位比较 |
| 组合文件 | `reports/portfolio-{YYYYMMDD}.md` | `reports/portfolio-mid-{YYYYMMDD}.md` |

**底仓/机动仓归属判定**（显式标注优先，无标注按规则推断并在报告标注"推断"）：

- 底仓候选：有深度研究报告 且 论文健康度 ≥ 7 且 PEG ≤ 1.2
- 机动仓：PEG > 1.5 或论文健康度 < 5，或卫星/试探性小仓位（≤8%）

**单只上限阶段判定**：**验证后** = 至少 2 个季度财务兑现（营收/利润增速达标）且论文健康度 ≥ 7；否则为**初始**。
...
### 示例4：中期组合审视（--horizon mid）
```
/portfolio-review 腾讯30%, 美团20%, 茅台20%, 英伟达15%, 现金15% --horizon mid
```
按中期规则审视：自动推断底仓/机动仓归属，校验底仓 60%~80% / 机动仓 20%~40%、单只/单行业上限，并按"3+2"结构给出集中度建议，输出 `reports/portfolio-mid-{YYYYMMDD}.md`。

### 示例5：中期组合审视（显式标注底仓/机动仓）
```
/portfolio-review 腾讯30%, 美团20%, 茅台20%, 英伟达15%, 现金15% --horizon mid 底仓标注：腾讯=底仓, 英伟达=机动仓
```
以用户显式标注为准判定底仓/机动仓归属（未标注标的按规则推断并在报告标注"推断"），其余同示例4。
...
| 中期组合报告 | `reports/portfolio-mid-{YYYYMMDD}.md` |
| 中期同日多次 | `reports/portfolio-mid-{YYYYMMDD}-v2.md`（递增版本号） |
