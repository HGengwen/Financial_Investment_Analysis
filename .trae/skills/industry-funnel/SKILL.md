...
4. **关键财务数据须从年报 PDF 一手数据源交叉验证**：使用 `report_hub.py ensure` 获取年报，**统一用** `tools/common/report_hub.py extract` 提取文字与表格（内部首选 pdf_extract.py，能自动还原财务附表），返回失败（退出码非0 / success=false / 扫描件）时才回退 Poppler 工具集，完整流程见 [pdf-extraction](../tools-scripts/pdf-extraction.md) 与下方"PDF 年报提取示例"
...
python tools/common/doubao_search.py "AI算力 全球竞争格局" --need-content --sites hkexnews.hk --export
python tools/common/tavily_search.py "AI算力 行业地位 竞争格局"

# 美股双源验证：exa（SEC filings 深度检索）+ doubao（新闻/舆情/分析师点评）
python tools/common/exa_search.py "AI semiconductor 10-K" --type deep --max-results 10
...
漏斗筛选的 5 条硬指标（PE/ROE/现金流/负债率）与财务质量分析，须从年报 PDF 一手数据源交叉验证。A股统一用 `tools/common/report_hub.py extract`（内部首选 pdf_extract.py，基于 pdf-inspector 库，能自动还原财务附表），返回失败（退出码非0 / success=false / 扫描件）时才回退 Poppler 工具集。完整规范见 [pdf-extraction](../tools-scripts/pdf-extraction.md) 与 [报告下载与提取统一入口](../tools-scripts/report-hub.md)。
