# 投资论文追踪 (Thesis Tracker)

买入后的纪律系统：建立投资论文、持续追踪检查、基于证据判断论文健康度，输出系统化的持有/调仓建议

---

## 快速开始

### 基本调用方式

```
/thesis-tracker {公司名}
```

### 长期模式（`--horizon long`，默认）

长期论文追踪，聚焦永续护城河与长期持有逻辑。`--horizon long` 为默认值，**可省略**；不传参数时行为与显式传 `--horizon long` 完全一致。

支持三种输入格式：

| 输入方式 | 示例                              | 说明                               |
| -------- | --------------------------------- | ---------------------------------- |
| 首次建立 | `/thesis-tracker 腾讯`          | 不存在论文文件时，进入建立论文模式 |
| 强制重建 | `/thesis-tracker 腾讯 建立论文` | 强制重新建立投资论文               |
| 季度检查 | `/thesis-tracker 腾讯 季度检查` | 基于最新财报进行论文检查           |

例如：

- `/thesis-tracker 腾讯`
- `/thesis-tracker 茅台 建立论文`
- `/thesis-tracker 英伟达 季度检查`

### 中期模式（`--horizon mid`）

中期论文追踪（1~3 年景气逻辑），与长期论文物理隔离：

| 输入方式 | 示例                                          | 说明                               |
| -------- | --------------------------------------------- | ---------------------------------- |
| 首次建立 | `/thesis-tracker 腾讯 --horizon mid`        | 不存在中期论文时，进入建立中期论文 |
| 强制重建 | `/thesis-tracker 腾讯 --horizon mid 建立论文` | 强制重建中期论文                   |
| 季度检查 | `/thesis-tracker 腾讯 --horizon mid 季度检查` | 基于最新财报的中期检查             |

---

## 核心功能

对指定公司执行投资论文追踪检查。从建立论文到持续追踪，系统化管理买入后的持有纪律，解决"该卖舍不得卖、不该卖恐慌卖出、忘记当初为什么买"三大痛点。

### 长期模式（`--horizon long`，默认）

长期论文追踪是默认行为，聚焦永续护城河、长期竞争力与买入后的持有纪律，论文文件输出至 `reports/{公司名}-thesis.md`。

#### 两种操作模式

1. **模式A：建立投资论文** — 首次使用时建立结构化论文，写入 `reports/{公司名}-thesis.md`
2. **模式B：追踪检查** — 后续使用时基于最新财报检查论文健康度

### 论文五要素

1. **核心论文（5句话）** — 必须用200字以内写清楚买入逻辑
2. **核心假设清单** — 拆解为3-7个可验证的具体假设
3. **红线清单** — 触发任何一条必须重新评估的致命条件
4. **估值锚点** — 买入时、乐观/中性/悲观目标价与安全边际
5. **追踪记录表** — 历次检查的健康度、核心变化、动作建议

### 建立论文六步流程

1. **A0 数据收集** — A股/港股/美股财务数据与行情获取
2. **A1 核心论文** — 5句话写清买入逻辑
3. **A2 核心假设清单** — 3-7个可验证假设
4. **A3 红线清单** — 致命/严重/警告三级红线
5. **A4 估值锚点** — 三情景估值（使用 `financial_rigor.py`）
6. **A5 保存论文** — 写入 `reports/{公司名}-thesis.md`

### 追踪检查七步流程

1. **B1 读取现有论文** — 加载核心论文、假设清单、红线清单
2. **B2 收集最新数据** — 最新财报、重大事件、股价估值、内部人交易
3. **B3 逐条检查核心假设** — 🟢成立 / 🟡边际弱化 / 🔴受损 / ⚫破裂
4. **B4 红线检查** — 逐条检查红线是否触发
5. **B5 估值更新** — 当前估值与买入时、上次检查对比
6. **B6 输出追踪报告** — 健康度评分 + 持有/加仓/减仓/清仓建议
7. **B7 更新论文文件** — 追加检查记录到追踪记录表

### 中期模式（`--horizon mid`）

中期模式切换中期规则，核心差异如下：

| 维度 | 长期（默认） | 中期（`--horizon mid`） |
| --- | --- | --- |
| 核心论文 | 永续护城河 5 问 | 1~3 年景气逻辑 5 问（赛道/渗透率/订单/估值/退出） |
| 假设清单 | 核心假设（3-7 个） | 中期假设（景气/渗透率/订单/估值/竞争格局） |
| 卖出检查 | 红线清单（致命/严重/警告） | P0~P5 六级检查（命中即停，禁止跳过 P0） |
| 健康度 | 长期公式 | 加入 PEG 水位扣分 + P0~P5 命中扣分 |
| 趋势判定 | — | 连续 3 季同向才确认趋势 |
| 论文文件 | `reports/{公司名}-thesis.md` | `reports/{公司名}/{公司名}-mid-thesis.md` |

**P0~P5 卖出检查表**：

| 级别 | 触发条件 | 决策 |
| --- | --- | --- |
| P0 治理一票否决 | 管理层失信、财务造假实锤、监管立案 | 立即清仓 |
| P1 硬止损 | 单笔亏损 8%~10%，无基本面反转 | 无条件止损 |
| P2 逻辑止损 | 连续 2 季低于预期；**或满 4 季度未兑现（时间止损）** | 减仓或清仓 |
| P3 产业景气拐点 | 渗透率减速 / ROE 拐头 | 减机动仓 |
| P4 估值透支 | PEG > 1.5（减机动仓）；PEG > 2（清机动仓） | 减/清机动仓 |
| P5 性价比替换 | 其他标的 PEG 显著更低且趋势更强 | 调仓（每季最多 1 次） |

---

## 使用示例

### 示例1：首次建立论文

```
/thesis-tracker 腾讯
```

不存在 `reports/腾讯-thesis.md` 时，进入建立论文模式，输出包含核心论文、假设清单、红线清单、估值锚点的完整论文文件。

### 示例2：季度追踪检查

```
/thesis-tracker 腾讯 季度检查
```

基于最新财报数据，逐条检查核心假设与红线，输出论文健康度评分与持有/调仓建议。

### 示例3：强制重建论文

```
/thesis-tracker 茅台 建立论文
```

强制重新建立投资论文，覆盖原有论文文件。

### 示例4：建立中期论文（--horizon mid）

```
/thesis-tracker 腾讯 --horizon mid
```

不存在 `reports/腾讯/腾讯-mid-thesis.md` 时，进入建立中期论文模式：输出 1~3 年景气逻辑中期论文（含中期假设清单、P0~P5 卖出检查表、PEG 水位锚点），写入 `reports/{公司名}/{公司名}-mid-thesis.md`，与长期论文物理隔离。

### 示例5：中期季度检查（--horizon mid）

```
/thesis-tracker 腾讯 --horizon mid 季度检查
```

基于最新财报逐条检查中期假设（含连续 3 季趋势判定）与 P0~P5 卖出检查，输出含 PEG 水位扣分的中期健康度评分与底仓/机动仓持有建议。

---

## 输出报告

报告将保存在以下位置：

| 报告类型     | 文件路径                                          |
| ------------ | ------------------------------------------------- |
| 投资论文     | `reports/{公司名}-thesis.md`                    |
| 追踪检查报告 | `reports/{公司名}-track-{YYYYMMDD}.md`（摘要追加到论文文件追踪记录表） |
| 中期论文     | `reports/{公司名}/{公司名}-mid-thesis.md`      |
| 中期检查报告 | `reports/{公司名}/{公司名}-mid-track-{YYYYMMDD}.md`（摘要追加到中期论文文件追踪记录表） |

### 论文文件结构

```markdown
# {公司名} 投资论文
**建立日期：{YYYY-MM-DD}**

## 核心论文（5句话）
我以 ___元 买入 ___公司，因为：
1. 这门生意的本质是___
2. 它的护城河是___
3. 管理层___
4. 当前价格相当于内在价值的___折
5. 即使我错了，下行风险可控，因为___

## 核心假设清单（3-7个可验证假设）
## 红线清单（致命/严重/警告三级）
## 估值锚点（买入时/乐观/中性/悲观）
## 追踪记录表（历次检查记录）
```

### 追踪报告结构

```markdown
# {公司名} 投资论文追踪报告
**报告生成日期：{YYYY-MM-DD}｜检查日期：{YYYY-MM-DD}**

## 一、论文健康度评分（X / 10分）
## 二、核心假设检查结果
## 三、红线检查结果
## 四、本期关键变化（不超过500字）
## 五、估值更新
## 六、结论与行动建议（持有/加仓/减仓/清仓）
## 七、下次检查需关注的重点
```

### 结论必须明确回答

1. **论文还完整吗？** 完整 / 边际弱化 / 受损 / 破裂
2. **该怎么做？** 加仓 / 持有 / 减仓 / 清仓
3. **下次检查时间**：下一个季报发布后 / 某个特定事件后

---

## 追踪标准

### 设计理念

大多数投资者的流程是：研究 → 买入 → 祈祷。缺少买入后的系统化跟踪。巴菲特和李录的做法是：**买入前就写下卖出条件**，然后每个季度检查论文是否完整。

### 论文健康度评分

**计算公式**：

```
健康度 = 10 - (⚫破裂假设数×3) - (🔴受损假设数×2) - (🟡弱化假设数×1) - (红线触发数×5)
最低1分，最高10分
```

| 评分 | 含义                           | 建议动作       |
| :--: | ------------------------------ | -------------- |
| 9-10 | 所有假设成立，论文比买入时更强 | 考虑加仓       |
| 7-8 | 核心假设成立，个别边际弱化     | 继续持有       |
| 5-6 | 1-2个假设受损，但核心逻辑未变  | 持有但提高警惕 |
| 3-4 | 多个假设受损，论文基础动摇     | 考虑减仓       |
| 1-2 | 红线触发或核心假设破裂         | 强烈建议卖出   |

### 中期健康度评分（`--horizon mid`）

**计算公式**：

```
中期健康度 = 10 - (⚫破裂假设×3) - (🔴受损假设×2) - (🟡弱化假设×1)
            - (P0~P5命中扣分) - (PEG水位扣分)
最低1分，最高10分
```

**P0~P5 命中扣分**（命中即停，只取最高优先级一项）：

| 命中级别 | 扣分 |
| --- | --- |
| P0 | 直接判 1 分（立即清仓） |
| P1 | -5 |
| P2 | -4 |
| P3 | -3 |
| P4 | -2 |
| P5 | -1 |
| 未命中 | 0 |

**PEG 水位扣分**（对齐 `/valuation-thermometer` 五档）：

| PEG 档位 | 区间 | 扣分 |
| --- | --- | --- |
| 显著低估 | < 0.8 | 0 |
| 合理 | 0.8 ~ 1.2 | 0 |
| 偏贵 | 1.2 ~ 1.5 | -1 |
| 高估 | > 1.5 | -2 |
| 严重透支 | > 2 | -3 |

| 评分 | 含义 | 建议动作 |
| :--: | --- | --- |
| 9-10 | 景气假设全部成立 + PEG 合理 | 底仓持有，机动仓可加 |
| 7-8 | 核心假设成立，个别边际弱化 | 底仓持有 |
| 5-6 | 假设受损或 PEG 偏贵 | 持有但减机动仓 |
| 3-4 | 多假设受损或 PEG 高估 | 减仓（先减机动仓） |
| 1-2 | P0~P2 命中或核心假设破裂 | 按 P0~P5 规则卖出 |

### 假设状态定义

| 状态                 | 含义                           |
| -------------------- | ------------------------------ |
| 🟢**成立**     | 最新数据支持该假设             |
| 🟡**边际弱化** | 数据仍在可接受范围，但趋势不利 |
| 🔴**受损**     | 数据明确不支持该假设           |
| ⚫**破裂**     | 假设已被推翻                   |

---

## 工具依赖

### 本地数据获取工具

根据上市地点选择相应的工具：

| 市场 | 工具                                  | 功能                      | 命令示例                                                       |
| ---- | ------------------------------------- | ------------------------- | -------------------------------------------------------------- |
| A股  | `tools/a_share/stock_info.py`       | 股票信息查询              | `python tools/a_share/stock_info.py --search 紫金矿业`       |
| A股  | `tools/a_share/stock_financial.py`  | 财务指标（ROE、毛利率等） | `python tools/a_share/stock_financial.py --code 601899`      |
| A股  | `tools/a_share/stock_quote.py`      | 历史股价                  | `python tools/a_share/stock_quote.py --code 601899`          |
| 港股 | `tools/hk_stock/stock_financial.py` | 港股信息与财务指标        | `python tools/hk_stock/stock_financial.py --financial 00700` |
| 港股 | `tools/hk_stock/stock_quote.py`     | 港股历史K线               | `python tools/hk_stock/stock_quote.py --code 00700`          |
| 美股 | `tools/us_stock/stock_info.py`      | 美股信息查询              | `python tools/us_stock/stock_info.py --search Apple`         |
| 美股 | `tools/us_stock/stock_financial.py` | 美股财务指标              | `python tools/us_stock/stock_financial.py --code AAPL`       |
| 美股 | `tools/us_stock/stock_quote.py`     | 美股行情数据              | `python tools/us_stock/stock_quote.py --code AAPL`           |

**Python路径**：`F:/Anaconda3/envs/Python_3_12_3/python.exe`

**数据源**：东方财富、新浪财经、巨潮资讯（A股）；东方财富、新浪财经（港股）；yfinance（美股）

详细使用说明请参考：

- **A股工具**：[docs/A股工具使用指南.md](../../docs/A股工具使用指南.md)
- **港股工具**：[docs/港股工具使用指南.md](../../docs/港股工具使用指南.md)
- **美股工具**：[docs/美股工具使用指南.md](../../docs/美股工具使用指南.md)
- **国际货币汇率**（跨市场估值/市值统一口径折算）：`tools/common/fx_rate.py`，详见 [A股工具使用指南汇率章节](../../docs/A股工具使用指南.md)

### 精确计算工具

| 工具                                | 功能                                          | 命令示例                                                                         |
| ----------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| `tools/common/financial_rigor.py` | 精确金融计算（PE、ROE、市值验证、三情景估值） | `python tools/common/financial_rigor.py verify-valuation --pe 25.5 --eps 10.2` |
| `tools/common/report_audit.py`    | 报告审核与数据抽检                            | `python tools/common/report_audit.py extract --report reports/xxx.md`          |

### 网络搜索工具

禁止使用 Anthropic 官方 WebSearch/WebFetch（中国大陆不可用），统一使用本地五工具组合。完整角色定位、市场×场景选型矩阵、命令速查、多源验证示例见 [web-search-tools](../tools-scripts/web-search-tools.md)。

**投资论文追踪场景下的搜索选型**：

- A股：`anysearch --tag finance` 主 + `doubao --finance` 辅
- 港股：`doubao --sites hkexnews.hk` 主 + `tavily` 辅；双源 doubao+tavily
- 美股：`exa --type deep` 主 + `doubao` 辅；双源 exa+doubao

**搜索规范**（论文追踪特有）：

- 时效性优先：使用 `--time-range month/week` 限定到本次与上次检查之间的区间
- 双源验证按市场矩阵执行：A股 anysearch+doubao；港股 doubao+tavily；美股 exa+doubao；
- 红线触发证据须来自监管披露/公司公告/财报原文
- 假设状态变化须标注证据来源日期，措辞变化不算状态变化
- 信息缺口标注"信息不足"，不得用搜索片段补造假设验证结论

**重要约束**：

- 网络搜索须使用本地五工具组合（详见 [web-search-tools](../tools-scripts/web-search-tools.md)）
- 估值数据须使用 `financial_rigor.py` 校验，禁止 LLM 心算
- 关键财务数据须至少两个来源交叉验证
- 港股 doubao+tavily、美股 exa+doubao 双源验证

---

## 核心原则

1. **买入前就写好卖出条件** — 冷静时做的决策比恐慌时做的好
2. **论文要具体到可验证** — "公司很好"不是论文，"ROE>25%且趋势稳定"才是
3. **红线一旦触发就行动** — 最怕的是"再等等看"，这是亏大钱的开始
4. **论文破裂 ≠ 股价下跌** — 股价跌30%不一定要卖，论文破裂才要卖
5. **诚实面对错误** — 论文建错了就承认，不要为了面子硬撑
6. **不预设立场** — 不因为持有就倾向于"论文完整"，证据指向哪边就写哪边
7. **数据准确性** — 关键数据至少两个来源交叉验证
8. **客观性原则** — 区分事实与观点，用数据支撑判断
9. **网络搜索时效性** — 使用 `--time-range month/week` 限制时间范围，优先获取最新信息
10. **非境内上市双源验证** — 港股/美股公司须按市场双源验证（港股 doubao+tavily；美股 exa+doubao）

---

## 注意事项

- 禁止使用 WebSearch 和 WebFetch 工具（中国大陆地区不可用）
- 估值数据须使用 `financial_rigor.py verify-valuation` 和 `three-scenario` 精确计算
- 关键财务数据必须至少两个来源交叉验证
- 论文核心必须用5句话写清楚，写不完整说明买入决策不够清晰
- 核心假设清单通常3-7个，太少说明思考不够深入，太多说明论文不够聚焦
- 红线触发时在报告中用醒目标注，给出明确的行动建议
- 不预设立场：先摆数据 → 推逻辑 → 出结论
- 网络搜索须使用 `--time-range month/week` 限制时间范围，优先获取最新信息
- 港股/美股公司须按市场双源验证（港股 doubao+tavily；美股 exa+doubao），确保信息准确性

---

## 局限性说明

- **数据可得性**：港股小票、新上市公司可能无法获取到完整信息
- **估值估算主观性**：内在价值估算存在主观判断，不同方法可能得出不同结论
- **假设验证滞后性**：某些假设需要多个季度才能验证，短期内难以判断
- **非实时数据**：工具获取的数据可能有延迟，不是实时数据
- **论文建立质量依赖**：如果初始论文建立质量不高，后续追踪效果会大打折扣
- 不构成投资建议，仅供学习研究参考

---

## 相关文档

- [SKILL.md](./SKILL.md) — 技能详细指令文件
- [论文漂移检测](../thesis-drift/README.md) — 对比新旧报告判定论文是否漂移
- [中期论文漂移检测](../mid-thesis-drift/README.md) — 中期论文的 H1~H6 漂移检测
- [投资团队](../investment-team/README.md) — 四Agent全面公司研究
- [财报团队](../earnings-team/README.md) — 六Agent团队精读 + 公众号发布
- [组合管理](../portfolio-review/README.md) — 组合层面审视与优化

---

## 版本信息

- **版本**：1.1.0
- **创建日期**：2026-07-26
- **更新日期**：2026-08-30（新增 `--horizon mid` 中期模式）
- **维护状态**：活跃维护

---

## 免责声明

本技能仅供学习研究参考，不构成投资建议。投资有风险，入市需谨慎。
