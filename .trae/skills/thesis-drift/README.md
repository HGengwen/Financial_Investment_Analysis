...
**搜索规范**（论文漂移检测特有）：
- 时效性优先：使用 `--time-range` 限定新旧报告日期之间的区间，避免混入旧证据
- 双源验证按市场矩阵执行：港股 doubao+tavily；美股 exa+doubao；A股 anysearch+doubao
- 触发漂移的证据须标注来源日期与发布主体，措辞改写不算证据
- 数值差异先用 `financial_rigor.py` 验算，再判定漂移方向
- 信息缺口标注"信息不足 / 无法判断"，不得用搜索片段补造旧论文基线
