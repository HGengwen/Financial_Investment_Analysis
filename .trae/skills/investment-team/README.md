...
7. **跨币种折算须用实时汇率**：跨市场估值对比、市值统一口径、财务数据折算（如港股/美股并入人民币口径）时，使用 `tools/common/fx_rate.py` 获取实时汇率（Akshare 优先 + yfinance 回退，支持 19 个货币对），勿用训练数据中的固定汇率；港股→人民币用 `HKDCNY`，美元→人民币用 `USDCNY`
