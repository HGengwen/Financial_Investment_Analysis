...
- **年报一手数据**：使用 `report_hub.py ensure` 获取年报，统一用 `report_hub.py extract` 提取（内部首选 pdf_extract.py，详见 [报告下载与提取统一入口](../tools-scripts/report-hub.md)），关键财务数据须从一手数据源交叉验证
...
| 年报一手数据 | `report_hub.py ensure` + [report-hub](../tools-scripts/report-hub.md) | 巨潮/HKEX披露易/SEC EDGAR |
