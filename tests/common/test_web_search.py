...
测试 tools/common/web_search.py 的功能。

用法:
python tests/common/test_web_search.py
"""
...
def test_import() -> None:
    """测试模块导入。"""
    try:
        from tools.common import web_search
...
async def test_search_function() -> Optional[Dict[str, Any]]:
    ...
    try:
        # 导入搜索函数
        from tools.common.web_search import search_web, DEFAULT_MCP_URL
...
def test_format_results() -> None:
    """测试结果格式化函数。"""
    try:
        from tools.common.web_search import format_search_results
