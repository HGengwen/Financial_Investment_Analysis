...
Usage:
    {py} -m pytest tests/common/test_stock_cache.py -v
    {py} tests/common/test_stock_cache.py
"""
...
class BaseCacheTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.patchers = [
            patch.object(stock_cache, "CODE_CACHE_FILE",
                         Path(self.tmp_path) / "stock_code.csv"),
            patch.object(stock_cache, "INDUSTRY_CACHE_FILE",
                         Path(self.tmp_path) / "stock_industry.csv"),
            patch.object(stock_cache, "STOCK_CACHE_TTL_DAYS", 7),
            patch.object(stock_cache, "_INDUSTRY_MIN_ROWS", 0),
            patch.object(stock_cache, "_INDUSTRY_MIN_VALID", 0),
            patch.object(stock_cache, "ak", MagicMock()),
        ]
        ...
        # 配置模拟 akshare 接口
        stock_cache.ak.stock_info_a_code_name.return_value = _fake_code_name_df()
        stock_cache.ak.stock_yjbb_em.return_value = _fake_yjbb_df()
    ...
    def _seed_code_cache(self) -> None:
        ...
        stock_cache._write_code_csv(records)
    ...
    def _seed_industry_cache(self) -> None:
        ...
        stock_cache._write_industry_csv(industry_map, "20260331")
...
class TestCodeNameList(BaseCacheTestCase):
    ...
    def test_hit_uses_cache_without_api(self) -> None:
        ...
        stock_cache.ak.stock_info_a_code_name.reset_mock()

        records = stock_cache.get_code_name_list()

        self.assertEqual(stock_cache.get_code_name_status(), "hit")
        stock_cache.ak.stock_info_a_code_name.assert_not_called()
        # 返回旧缓存中的 2 条数据
    ...
    def test_expired_cache_triggers_refresh(self) -> None:
        ...
        self._seed_code_cache()
        self._backdate_file(stock_cache.CODE_CACHE_FILE)
        stock_cache.ak.stock_info_a_code_name.reset_mock()

        records = stock_cache.get_code_name_list()

        self.assertEqual(stock_cache.get_code_name_status(), "refresh")
        stock_cache.ak.stock_info_a_code_name.assert_called_once()
        # 刷新后缓存为 API 返回的 3 条全量数据
        self.assertEqual(len(records), 3)
        self.assertIn({"code": "300502", "name": "新易盛", "market": "a"}, records)
        # 本地缓存文件已覆写
        cached = stock_cache._read_code_csv()
    ...
    def test_force_refresh_ignores_fresh_cache(self) -> None:
        ...
        records = stock_cache.get_code_name_list(force_refresh=True)
    ...
    def test_corrupt_csv_falls_back_to_api(self) -> None:
        ...
        # 写入可解析但缺少必需列的无效 CSV
        with open(stock_cache.CODE_CACHE_FILE, "w", encoding="utf-8") as f:
            f.write("foo,bar\n1,2\n")
        stock_cache.ak.stock_info_a_code_name.reset_mock()
    ...
    def test_atomic_write_leaves_no_tmp(self) -> None:
        """原子写入后无 .tmp 临时文件残留。"""
        self._seed_code_cache()
        tmp_path = stock_cache.CODE_CACHE_FILE.with_suffix(
            stock_cache.CODE_CACHE_FILE.suffix + ".tmp")
        self.assertFalse(tmp_path.exists())
        self.assertTrue(stock_cache.CODE_CACHE_FILE.exists())

    def test_refresh_failure_uses_stale_cache(self) -> None:
        """刷新失败（限流/断连）时降级返回旧缓存并标注 stale。"""
        self._seed_code_cache()
        self._backdate_file(stock_cache.CODE_CACHE_FILE)
        stock_cache.ak.stock_info_a_code_name.side_effect = RuntimeError("限流")

        records = stock_cache.get_code_name_list()

        self.assertEqual(stock_cache.get_code_name_status(), "stale")
        # 返回旧缓存的 2 条数据
        self.assertEqual(len(records), 2)

    def test_no_cache_and_fetch_failure_raises(self) -> None:
        """无缓存且刷新失败时抛出异常。"""
        stock_cache.ak.stock_info_a_code_name.side_effect = RuntimeError("断连")
        with self.assertRaises(RuntimeError):
            stock_cache.get_code_name_list()
...
class TestIndustryMap(BaseCacheTestCase):
    ...
    def test_industry_hit_uses_cache(self) -> None:
        ...
        stock_cache.ak.stock_yjbb_em.reset_mock()

        result = stock_cache.get_industry_map()

        self.assertEqual(stock_cache.get_industry_status(), "hit")
        stock_cache.ak.stock_yjbb_em.assert_not_called()
    ...
    def test_industry_quarter_fallback(self) -> None:
        ...
        stock_cache.ak.stock_yjbb_em.side_effect = side_effect

        result = stock_cache.get_industry_map()

        self.assertEqual(stock_cache.get_industry_status(), "refresh")
    ...
    def test_industry_all_quarter_fail_raises_without_cache(self) -> None:
        ...
        stock_cache.ak.stock_yjbb_em.side_effect = RuntimeError("全部失败")
        with self.assertRaises(RuntimeError):
            stock_cache.get_industry_map()

    def test_industry_refresh_failure_uses_stale_cache(self) -> None:
        """行业缓存刷新失败时降级返回旧缓存并标注 stale。"""
        self._seed_industry_cache()
        self._backdate_file(stock_cache.INDUSTRY_CACHE_FILE)
        stock_cache.ak.stock_yjbb_em.side_effect = RuntimeError("限流")

        result = stock_cache.get_industry_map()

        self.assertEqual(stock_cache.get_industry_status(), "stale")
        self.assertEqual(result["300502"]["industry"], "通信设备")
    ...
    def test_industry_refresh_overwrites_cache(self) -> None:
        ...
        self.assertEqual(stock_cache.get_industry_status(), "refresh")
        stock_cache.ak.stock_yjbb_em.assert_called_once()
        # 刷新后为 API 返回的 3 条数据
